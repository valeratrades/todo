...
fn test_exact_match_with_extension_skips_fzf() {
	...
	let output = ctx.run(&["blocker", "--individual-files", "set", "uni.md"]);
...
fn test_unique_pattern_without_extension_matches_directly() {
	...
	let output = ctx.run(&["blocker", "--individual-files", "set", "alpha"]);
	assert!(output.status.success());
...
fn test_set_project_cannot_switch_away_from_urgent() {
	...
	let output = ctx.run(&["blocker", "--individual-files", "set", "work/urgent.md"]);
	assert!(output.status.success());
	assert_eq!(ctx.read_current_project(), Some("work/urgent.md".to_string()));

	// Now try to switch away from urgent - should be blocked
	let output = ctx.run(&["blocker", "--individual-files", "set", "work/normal.md"]);
	assert!(output.status.success());
...
fn test_set_project_can_switch_between_urgent_files() {
	...
	let output = ctx.run(&["blocker", "--individual-files", "set", "personal/urgent.md"]);
	assert!(output.status.success());
...
fn test_can_add_to_same_urgent_file() {
	...
	let output = ctx.run(&["blocker", "--individual-files", "add", "--urgent", "another urgent task"]);
	assert!(output.status.success());
